...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if type(operator) is MacroProcedure : #apply macro procedure
...
def eval_all(expressions, env):
    ...
    if expressions == nil :
        return None
    if expressions.second == nil : #reached last element
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals) :
            child_frame = Frame(self) #create child_frame and the partent frame is self
            while formals is not nil :
                child_frame.define(formals.first,vals.first) #add formals and values to child_frame
                formals=formals.second
                vals=vals.second
            return child_frame
        else :
            raise SchemeError('different number of arguments in formals and vals')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try :
            ...
            raise SchemeError('{} wrong number of arguments'.format(e))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lam = LambdaProcedure(formal,body,env)
        env.define(name, lam)
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil :
        return True
    elif expressions.second == nil : #return last true value
        return scheme_eval(expressions.first,env,True)
    elif scheme_falsep(scheme_eval(expressions.first,env)) : #return false value
        return False
    else :
...
def do_or_form(expressions, env):
    ...
    if expressions == nil :
        return False
    elif scheme_truep(scheme_eval(expressions.first,env, not expressions.second)) :
        return scheme_eval(expressions.first,env, not expressions.second)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            if len(clause) >= 2: #multiple sub-expression
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil :
        ...
        check_formals(name)
        expression = scheme_eval(bindings.first.second.first,env)
        names= Pair(name,names)
        expressions = Pair(expression,expressions)
        bindings = bindings.second
    return env.make_child_frame(names,expressions)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target,Pair) :
        name = target.first #name of the function
        if scheme_symbolp(name) :
            formal = target.second
            body = expressions.second
            env.bindings[name] = MacroProcedure(formal,body,env)
            return name
    raise SchemeError('Symbol form is incorrect')
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, parent):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamiclly-scoped call evaluated in environment parent."""
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
