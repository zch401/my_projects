...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None :
            return False
        if self.place.ant.block_path == False :
            return False
...
class ThrowerAnt(Ant):
    ...
    food_cost=3
    min_range=0 #default value for min_range is zero for ThrowerAnt
    max_range=float('inf') #defaule value for max_range is infinity for ThrowerAnt
    def nearest_bee(self, hive):
        ...
        target_place=self.place.entrance#the first place in front the ant's current place

        step=1 #how many steps is target_place from the place of ant
        while target_place is not hive :
            if(len(target_place.bees)!=0) and (step>=self.min_range and step <=self.max_range) :
                return random_or_none(target_place.bees)
            target_place=target_place.entrance #no bee in this place, we advance to next
            step+=1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        if self.armor-amount <= 0 : # check if fire ant's armor is smaller than 0
            temp_list_bee=list(self.place.bees) #list of bees will be damaged
            for target_bee in temp_list_bee :
                target_bee.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor) # take out the bee
        # END Problem 6

    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting == 0: #not digesting
            if len(self.place.bees) != 0 : #there are bees at the ant's place
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        target_bees=list(self.place.bees) #list of bees will pass by
        for target_bee in target_bees : #each bee from the bees list
            target_bee.reduce_armor(self.damage) #deal damage to each bee
        # END Problem 7

# BEGIN Problem 8
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does not do anything but has a large amor value"""
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant != None :# if there is a contained ant, that ant will take action
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_watersafe == False :
            insect.reduce_armor(insect.armor) #if the insect is not watersafe, kill the insect immediately
        # END Problem 11

# BEGIN Problem 12
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can be placed on water and has identical
    function as ThrowerAnt.
    """
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.is_true_queen == False : #if not true queen, die immediately
            self.reduce_armor(self.armor)
            return
        ScubaThrower.action(self,colony)
        position = self.place.exit
        while position is not None : #position does not reach end
            if position.ant :
                if position.ant not in self.doubled : #only double damage of ants not been doubled before
                    position.ant.damage *=2
                    self.doubled.append(position.ant)
                if position.ant.is_container and position.ant.contained_ant : #check if contained ant was doubled
                    if position.ant.contained_ant not in self.doubled :
                        position.ant.contained_ant.damage *=2
                        self.doubled.append(position.ant.contained_ant)
...
def make_slow(action, bee):
    ...
    def new_action(colony) :
        if (colony.time % 2) == 0 : #bee only take action when time is even
            action(colony)
        else :
            return
    return new_action
...
def make_scare(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    bee.is_scared = True
    bee.reverse = True #bee is going back
    def new_action(colony) :
        if bee.place.entrance is colony.hive :
            return
        action(colony)
    return new_action
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    def new_action (colony) :
        ...
        if duration > 0 :
            ...
            bee.reverse = False #bee take normal action, set direction back
            old_action(colony)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        while location is not hive :
            #bee list
            if len(location.bees) :
                for bee in location.bees :
                    insect_distance[bee] = distance
            #ant list
            if location.ant is not None :
                insect_distance[location.ant] = distance
                if location.ant.is_container :
                    insect_distance[location.ant.contained_ant] = distance
            location = location.entrance # go to next location
            distance += 1
        insect_distance.pop(self) #no damage for LaserAnt self
        return insect_distance
        # END Problem OPTIONAL

    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        base_damage = self.energy - 0.05 * self.insects_shot
        if base_damage <= 0 :# if the LaserAnt used up its energy
            return 0
        damage = base_damage - 0.2 * distance
        return damage
        # END Problem OPTIONAL
