...
def group_by_centroid(restaurants, centroids):
    ...
    for x in restaurants :
        pair= []
        pair+=[find_closest(restaurant_location(x),centroids),x]
        list+=[pair]
    #print(group_by_first(list)[0])
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    avg_latitude=mean([restaurant_location(x)[0] for x in cluster])
    avg_longitude=mean([restaurant_location(x)[1] for x in cluster])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xx=0
    for x in xs :
        S_xx+=(x-xs_avg)*(x-xs_avg)
    ys_avg=mean(ys)
    S_yy=0
    for y in ys :
        S_yy+=(y-ys_avg)*(y-ys_avg)
    list=zip(xs,ys)
    S_xy=0
    for xy in list :
        S_xy+=(xy[0]-xs_avg)*(xy[1]-ys_avg)
    b=S_xy/S_xx
    a=ys_avg-b*xs_avg
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    list=[find_predictor(user,reviewed,ff) for ff in feature_fns]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants :
        if restaurant in reviewed :
            name_rating[restaurant_name(restaurant)]=user_rating(user,restaurant_name(restaurant))
        if restaurant not in reviewed :
            name_rating[restaurant_name(restaurant)]=predictor(restaurant)
    return name_rating
    # END Question 9
